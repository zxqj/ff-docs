...
tmux send-keys -t 'server-log' 'docker run --detach --rm -p 8888:8888 -v .:/home/jovyan/work --name jupyter-wiki quay.io/jupyter/base-notebook:2024-11-19 jupyter lab --ServerApp.token="" && sleep 2s && xdg-open http://localhost:8888 && docker attach jupyter-wiki' C-m
